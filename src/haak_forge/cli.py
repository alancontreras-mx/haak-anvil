"""Haak Forge CLI — entry point for `haak-forge` shell command."""
...
from haak_forge import __version__
...
from haak_forge.renderers import get_renderer

app = typer.Typer(
    name="haak-forge",
    help="Modern multi-format pentest report generator. "
    "https://github.com/HaakConsulting/haak-forge",
...
@app.command()
def version() -> None:
    """Print Haak Forge version."""
    console.print(f"haak-forge {__version__}")
...
def merge(
    inputs: Annotated[
        list[Path],
        typer.Argument(help="JSON bundles previously generated by haak-forge"),
    ],
    engagement: Annotated[
        Path | None,
    ...
    """Merge multiple haak-forge JSON bundles into one report."""
    if len(inputs) < 2:
